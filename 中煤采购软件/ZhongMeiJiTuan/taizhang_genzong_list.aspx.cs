...
public partial class taizhang_genzong_list : System.Web.UI.Page
{
...
where isdelete=0 and isfinished=0 " + ViewState["search"] + " group by dingdanbianhao,gonghuochangshang,dinghuoyiju,caigoufangshi,dingdanriqi,b.jihuabianhao,b.yingdaoshijian order by b.yingdaoshijian asc";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
    ...
    protected void lbtn_search_Click(object sender, EventArgs e)
    {
        ...
        {
            ViewState["search"] += " and b.changshangmingcheng like'%" + changshangmingcheng + "%'";
        }
        BindInfo();
    }
    ...
    protected void lbtn_export_all_Click(object sender, EventArgs e) {

        string sql = "select *,DATEDIFF(d,b.yingdaoshijian,getdate()) yuqitianshu from taizhang_info b where 1=1 " + ViewState["search"];
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);

        //模板文件  
        string TempletFileName = Server.MapPath("~/template/tyanqibiao.xlsx");//路径 
        //导出文件  
        string dts = DateTime.Now.ToString("yyyyMMddHHmmss");
        string ReportFileName = Server.MapPath("~/xiazai/yanqijiaohuomingxibiao" + dts + ".xlsx");
        Microsoft.Office.Interop.Excel.Application myExcel = new Microsoft.Office.Interop.Excel.Application();
        object oMissing = System.Reflection.Missing.Value;
        myExcel.Application.Workbooks.Open(TempletFileName, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing, oMissing);
        Microsoft.Office.Interop.Excel.Workbook myBook = myExcel.Workbooks[1];
        ...
        for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
        {
            mySheet.Cells[i + 3, 1] = (i + 1).ToString();
            mySheet.Cells[3 + i, 2] = ds.Tables[0].Rows[i]["shiyongdanwei"].ToString();
            mySheet.Cells[3 + i, 3] = ds.Tables[0].Rows[i]["caigoubianhao"].ToString();
            mySheet.Cells[3 + i, 4] = ds.Tables[0].Rows[i]["dingjiawuzimingcheng"].ToString();
            mySheet.Cells[3 + i, 5] = ds.Tables[0].Rows[i]["dingjiaguigexinghao"].ToString();
            mySheet.Cells[3 + i, 6] = ds.Tables[0].Rows[i]["dingjiadanwei"].ToString();
            mySheet.Cells[3 + i, 7] = ds.Tables[0].Rows[i]["dingjiashuliang"].ToString();
            mySheet.Cells[3 + i, 8] = ds.Tables[0].Rows[i]["hanshuidanjia"].ToString();
            mySheet.Cells[3 + i, 9] = ds.Tables[0].Rows[i]["hanshuijine"].ToString();
            mySheet.Cells[3 + i, 10] = ds.Tables[0].Rows[i]["gonghuochangshang"].ToString();
            mySheet.Cells[3 + i, 11] = ds.Tables[0].Rows[i]["yingdaoshijian"].ToString();
            mySheet.Cells[3 + i, 12] = ds.Tables[0].Rows[i]["yuqitianshu"].ToString();
        }
        ...
        Response.ContentEncoding = System.Text.Encoding.UTF8;
        // 添加头信息，为"文件下载/另存为"对话框指定默认文件名   
        Response.AddHeader("Content-Disposition", "attachment; filename=" + Server.UrlEncode("延期交货明细表.xlsx"));
        // 添加头信息，指定文件大小，让浏览器能够显示下载进度   
        Response.AddHeader("Content-Length", filet.Length.ToString());
        // 指定返回的是一个不能被客户端读取的流，必须被下载   
        Response.ContentType = "application/ms-excel";
    ...
    protected void lbtn_export_Click(object sender, EventArgs e) {
        ...
        string sql = "select shiyongdanwei,caigoubianhao,dingjiawuzimingcheng,dingjiaguigexinghao,dingjiadanwei,dingjiashuliang,hanshuidanjia,hanshuijine,gonghuochangshang,yingdaoshijian,DATEDIFF(d,yingdaoshijian,getdate()) yuqitianshu from taizhang_info  where dingdanbianhao in(" + ids + ")";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
