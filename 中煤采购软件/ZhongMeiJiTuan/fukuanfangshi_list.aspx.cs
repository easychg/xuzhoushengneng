...
public partial class fukuanfangshi_list : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from fukuanfangshi where " + ViewState["search"] + " order by addtime desc";
    ...
    protected void lbtn_save_Click(object sender, EventArgs e)
    {
        ...
        if (sydw == "")
        {
            SystemTool.AlertShow(this, "请输入付款方式");
            return;
        }
        string sql = "select count(fukuanfangshi_id) from fukuanfangshi where mingcheng='" + sydw + "'";
        ...
        if (result != "" && result != "no")
        {
            ...
            {
                //未查询
                sql = "insert into fukuanfangshi(mingcheng) values('" + sydw + "')";
    ...
    protected void lbtn_dels_Click(object sender, EventArgs e)
    {
        ...
        string sql = "delete from fukuanfangshi where fukuanfangshi_id in(" + ids + ")";
    ...
    protected void lbtn_cancel_Click(object sender, EventArgs e)
    {
        ...
        string sql = "update fukuanfangshi set isxianzhi=0 where fukuanfangshi_id in(" + ids + ")";
        int result = DB.ExecuteSql(sql, null);
        if (result > 0)
        {
            BindInfo();
    ...
    protected void lbtn_add_Click(object sender, EventArgs e)
    {
        ...
        string sql = "update fukuanfangshi set isxianzhi=1 where fukuanfangshi_id in(" + ids + ")";
        int result = DB.ExecuteSql(sql, null);
        if (result > 0)
        {
            BindInfo();
