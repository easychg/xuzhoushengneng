...
public partial class dianshangpingtai_list : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from dianshangpingtai where " + ViewState["search"] + " order by addtime desc";
    ...
    protected void lbtn_save_Click(object sender, EventArgs e)
    {
        ...
        if (sydw == "")
        {
            SystemTool.AlertShow(this, "请输入电商平台");
            return;
        }
        string sql = "select count(dianshangpingtai_id) from dianshangpingtai where mingcheng='" + sydw + "'";
        ...
        if (result != "" && result != "no")
        {
            ...
            {
                //未查询
                sql = "insert into dianshangpingtai(mingcheng) values('" + sydw + "')";
    ...
    protected void lbtn_dels_Click(object sender, EventArgs e)
    {
        ...
        string sql = "delete from dianshangpingtai where dianshangpingtai_id in(" + ids + ")";
    ...
    protected void lbtn_cancel_Click(object sender, EventArgs e)
    {
        ...
        string sql = "update dianshangpingtai set isxianzhi=0 where dianshangpingtai_id in(" + ids + ")";
        int result = DB.ExecuteSql(sql, null);
        if (result > 0)
        {
            BindInfo();
    ...
    protected void lbtn_add_Click(object sender, EventArgs e)
    {
        ...
        string sql = "update dianshangpingtai set isxianzhi=1 where dianshangpingtai_id in(" + ids + ")";
        int result = DB.ExecuteSql(sql, null);
        if (result > 0)
        {
            BindInfo();
