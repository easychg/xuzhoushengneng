...
public partial class shiyongdanwei_list : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from shiyongdanwei where " + ViewState["search"] + " order by addtime desc";
    ...
    protected void lbtn_save_Click(object sender, EventArgs e)
    {
        ...
        string sql = "select count(shiyongdanwei_id) from shiyongdanwei where mingcheng='" + sydw + "'";
        string result = DB.ExecuteSqlValue(sql, null);
        if (result != "" && result != "no") {
            if (result == "0")
            {
                //未查询
                sql = "insert into shiyongdanwei(mingcheng) values('" + sydw + "')";
    ...
    protected void lbtn_dels_Click(object sender, EventArgs e)
    {
        ...
        string sql = "delete from shiyongdanwei where shiyongdanwei_id in(" + ids + ")";
    ...
    protected void lbtn_cancel_Click(object sender, EventArgs e)
    {
        ...
        string sql = "update shiyongdanwei set isxianzhi=0 where shiyongdanwei_id in(" + ids + ")";
        int result = DB.ExecuteSql(sql, null);
        if (result > 0)
        {
            BindInfo();
