...
public partial class jiaohuodidian : System.Web.UI.Page
{
    ...
    {
        string sql = "select * from jiaohuodidian where " + ViewState["search"] + " order by addtime desc";
    ...
    protected void lbtn_save_Click(object sender, EventArgs e)
    {
        ...
        if (sydw == "")
        {
            SystemTool.AlertShow(this, "请输入交货地点");
            return;
        }
        string sql = "select count(jiaohuodidian_id) from jiaohuodidian where mingcheng='" + sydw + "'";
        ...
        if (result != "" && result != "no")
        {
            ...
            {
                //未查询
                sql = "insert into jiaohuodidian(mingcheng) values('" + sydw + "')";
    ...
    protected void lbtn_dels_Click(object sender, EventArgs e)
    {
        ...
        string sql = "delete from jiaohuodidian where jiaohuodidian_id in(" + ids + ")";
    ...
    protected void lbtn_cancel_Click(object sender, EventArgs e)
    {
        ...
        string sql = "update jiaohuodidian set isxianzhi=0 where jiaohuodidian_id in(" + ids + ")";
        int result = DB.ExecuteSql(sql, null);
        if (result > 0)
        {
            BindInfo();
    ...
    protected void lbtn_add_Click(object sender, EventArgs e)
    {
        ...
        string sql = "update jiaohuodidian set isxianzhi=1 where jiaohuodidian_id in(" + ids + ")";
        int result = DB.ExecuteSql(sql, null);
        if (result > 0)
        {
            BindInfo();
