...
public partial class manager_albumadd : System.Web.UI.Page
{
    ...
    protected void btnok_Click(object sender, EventArgs e)
    {
        ...
        string sql = "insert into article_info(nav_id,title,article_image,author,descr,detail,isshow) values(@nav_id,@title,@article_image,@author,@descr,@detail,@isshow)";
        if (Request.QueryString["id"] != null)
        {
            sql = "update article_info set nav_id=@nav_id,title=@title,article_image=@article_image,author=@author,descr=@descr,detail=@detail,isshow=@isshow where id=" + Request.QueryString["id"].ToString();
        }
        ...
        if (result > 0)
        {
            SystemTool.AlertShow_Refresh(this, "保存成功", "albumlist.aspx");
        }
        else
        {
