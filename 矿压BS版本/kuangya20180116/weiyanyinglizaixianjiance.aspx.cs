...
public partial class weiyanyinglizaixianjiance : System.Web.UI.Page
{
    ...
    private void BindData()
    {
        ...
        //    sql = "select * from  DrillPar where areaName='" + areaname + "' and roadwayName='" + ds.Tables[0].Rows[0]["roadName"].ToString() + "'";
        //    ds = DB.ExecuteSqlDataSet(sql, null);
        //    if (ds.Tables[0].Rows.Count > 0) {
        //        txtchuanganqizushu.Value = ds.Tables[0].Rows[0]["sennumber"].ToString();
        //        txtyinglibaojingzhi.Value = ds.Tables[0].Rows[0]["pressalarm"].ToString();
        //    }
        //}
    ...
    protected void ddl_cequ_changed(object sender, EventArgs e)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            sql = "select * from  DrillPar where areaName='" + ddl_cequ.SelectedValue + "' and roadwayName='" + roadwayname + "'";
            ds = DB.ExecuteSqlDataSet(sql, null);
            if (ds.Tables[0].Rows.Count > 0)
            {
                txtchuanganqizushu.Value = ds.Tables[0].Rows[0]["sennumber"].ToString();
            ...
            sql = @"select count(*) as total from DrillSenInfo1 dsi
...
where dsi.areaname='" + ddl_cequ.SelectedValue + "' and dsi.roadwayname='" + roadwayname + "' ";
    ...
    protected void ddl_xiangdao_changed(object sender, EventArgs e)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            sql = "select * from  DrillPar where areaName='" + ddl_cequ.SelectedValue + "' and roadwayName='" + ddl_xiangdao.SelectedValue + "'";
            ds = DB.ExecuteSqlDataSet(sql, null);
            if (ds.Tables[0].Rows.Count > 0)
            {
                txtchuanganqizushu.Value = ds.Tables[0].Rows[0]["sennumber"].ToString();
            ...
            sql = @"select count(*) as total from DrillSenInfo1 dsi
left join DrillNewData dnd on dsi.areaname=dnd.areaname and dsi.roadwayname=dnd.roadwayname and dsi.sensorno=dnd.sensorno
where dsi.areaname='" + ddl_cequ.SelectedValue + "' and dsi.roadwayname='" + ddl_xiangdao.SelectedValue + "' ";
            ds = DB.ExecuteSqlDataSet(sql, null);
            decimal total = Convert.ToDecimal(ds.Tables[0].Rows[0]["total"].ToString());
            ddl_page.Items.Clear();
            Int32 p = 16;//每页条数
