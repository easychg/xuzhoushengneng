...
public partial class huozhusuoliangshujuchaxun : System.Web.UI.Page
{
    ...
    {
        string sql = @"SELECT p.AreaName,p.FaceName,BracketNo,convert(varchar(100),time,20) as time,p.sensorNo,HuoZhu1,AlarmState FROM HuoZhuData p inner join HuoZhuSenInfo i on p.SensorNo=i.SensorNo where " + ViewState["search"] + " order by time desc,SensorNo";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
    ...
    protected void search_Click(object sender, EventArgs e)
    {
        ...
        if (roadway != "0")
        {
            ViewState["search"] += " and p.FaceName='" + roadway + "'";
        }
        if (anzhuangweizhi != "0")
        {
            ViewState["search"] += " and BracketNo='" + anzhuangweizhi + "'";
        }
        BindInfo();
    }
    protected void lbtn_export_Click(object sender, EventArgs e)
    {
        string sql = @"SELECT p.AreaName,p.FaceName,BracketNo,convert(varchar(100),time,20) as time,p.sensorNo,HuoZhu1,AlarmState FROM HuoZhuData p inner join HuoZhuSenInfo i on p.SensorNo=i.SensorNo where " + ViewState["search"] + " order by time desc,SensorNo";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
        string[] columns = { "测区名称", "工作面名称","支架号", "时间", "传感器编号", "缩量值（mm）", "报警状态" };
        ...
        book.Write(ms);
        HttpContext.Current.Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}.xls", "活柱缩量数据查询"));
        HttpContext.Current.Response.BinaryWrite(ms.ToArray());
        book = null;
        ms.Close();
        ms.Dispose();
        ////模板文件  
        //string TempletFileName = Server.MapPath("~/template/huozhusuoliangshujuchaxun.xlsx");//路径 
        ////导出文件  
        //string dts = DateTime.Now.ToString("yyyyMMddHHmmss");
        //string ReportFileName = Server.MapPath("~/xiazai/huozhusuoliangshujuchaxun" + dts + ".xlsx");
        ...
        //    mySheet.Cells[2 + i, 2] = ds.Tables[0].Rows[i]["facename"].ToString();
        //    mySheet.Cells[2 + i, 3] = ds.Tables[0].Rows[i]["time"].ToString();
        //    mySheet.Cells[2 + i, 4] = ds.Tables[0].Rows[i]["sensorNo"].ToString();
        //    mySheet.Cells[2 + i, 5] = ds.Tables[0].Rows[i]["bracketno"].ToString();
        //    mySheet.Cells[2 + i, 6] = ds.Tables[0].Rows[i]["huozhu1"].ToString();
        //    mySheet.Cells[2 + i, 7] = ds.Tables[0].Rows[i]["alarmstate"].ToString();
        //}
        ...
        //Response.AddHeader("Content-Disposition", "attachment; filename=" + Server.UrlEncode("活柱缩量数据查询表.xlsx"));
    ...
    protected void ddl_cequ_changed(object sender, EventArgs e)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            sql = "SELECT BracketNo FROM HuoZhuSenInfo where areaName='" + ddl_cequ.SelectedValue + "' and FaceName='" + ds.Tables[0].Rows[0]["workfaceName"].ToString() + "' order by BracketNo asc";
    ...
    {
        string sql = "SELECT BracketNo FROM HuoZhuSenInfo where areaName='" + ddl_cequ.SelectedValue + "' and FaceName='" + ddl_roadway.SelectedValue + "' order by BracketNo asc";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
        if (ds.Tables[0].Rows.Count > 0)
        {
            ddl_anzhuangweizhi.Items.Clear();
