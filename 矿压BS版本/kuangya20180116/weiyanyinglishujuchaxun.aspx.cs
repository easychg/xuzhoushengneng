...
public partial class weiyanyinglishujuchaxun : System.Web.UI.Page
{
    ...
    {
        string sql = @"select Location,b.AreaName,b.roadwayName,convert(varchar(100),time,20) as time,Depth,b.sensorNo,Stress,(case AlarmH when 'false' then '正常' when'true' then '报警' end) as AlarmH from DrillData b inner join DrillSenInfo1 bi on b.sensorNo=bi.sensorNo where " + ViewState["search"] + " order by time desc,Location";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
    ...
    {
        string sql = @"select Location,b.AreaName,b.roadwayName,convert(varchar(100),time,20) as time,b.sensorNo,Stress,(case AlarmH when 'false' then '正常' when'true' then '报警' end) as AlarmH from DrillData b inner join DrillSenInfo1 bi on b.sensorNo=bi.sensorNo where " + ViewState["search"] + " order by time desc,Location";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
        ...
        book.Write(ms);
        HttpContext.Current.Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}.xls", "围岩应力数据查询"));
        HttpContext.Current.Response.BinaryWrite(ms.ToArray());
        book = null;
        ms.Close();
        ms.Dispose();
        ////模板文件  
        //string TempletFileName = Server.MapPath("~/template/weiyanyinglishujuchaxun.xlsx");//路径 
        ////导出文件  
        //string dts = DateTime.Now.ToString("yyyyMMddHHmmss");
        //string ReportFileName = Server.MapPath("~/xiazai/weiyanyinglishujuchaxun" + dts + ".xlsx");
        ...
        //    mySheet.Cells[2 + i, 6] = ds.Tables[0].Rows[i]["depth"].ToString();
        //    mySheet.Cells[2 + i, 7] = ds.Tables[0].Rows[i]["stress"].ToString();
        //    mySheet.Cells[2 + i, 8] = ds.Tables[0].Rows[i]["alarmh"].ToString();
        //}
        ...
        //Response.AddHeader("Content-Disposition", "attachment; filename=" + Server.UrlEncode("围岩应力数据查询表.xlsx"));
    ...
    protected void ddl_cequ_changed(object sender, EventArgs e)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            sql = "SELECT Location FROM DrillSenInfo1 where areaName='" + ddl_cequ.SelectedValue + "' and roadwayName='" + ds.Tables[0].Rows[0]["roadName"].ToString() + "' order by Location asc";
    ...
    {
        string sql = "SELECT Location FROM DrillSenInfo1 where areaName='" + ddl_cequ.SelectedValue + "' and roadwayName='" + ddl_roadway.SelectedValue + "' order by Location asc";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
        if (ds.Tables[0].Rows.Count > 0)
        {
            ddl_anzhuangweizhi.Items.Clear();
