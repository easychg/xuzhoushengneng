...
public partial class dingbanlicengshujuchaxun : System.Web.UI.Page
{
    ...
    {
        string sql = "SELECT Location,dd.AreaName,dd.roadwayName,convert(varchar(100),time,20) as time,dd.sensorNo,Displacement1,Displacement2,PointdepthA,PointdepthB,convert(decimal(18,2), ABS(Displacement2-Displacement1)) as ABSWY,AlarmH FROM DisplacementData dd inner join DisSenInfo di on dd.sensorNo=di.sensorNo where " + ViewState["search"] + " order by time desc , Location";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
    ...
    protected void search_Click(object sender, EventArgs e)
    {
        ...
        if (cequ != "0")
        {
            ViewState["search"] += " and dd.areaName='" + cequ + "'";
        }
        if (roadway != "0")
        {
            ViewState["search"] += " and dd.roadwayName='" + roadway + "'";
        }
        if (anzhuangweizhi != "0")
        {
    ...
    protected void lbtn_export_Click(object sender, EventArgs e)
    {
        ...
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
        string[] columns = { "测区名称","巷道名称","时间","传感器编号","位置","A基点深度","B基点深度","A(mm)","B(mm)","|B-A|(mm)","报警状态"};
        ...
        book.Write(ms);
        HttpContext.Current.Response.AddHeader("Content-Disposition", string.Format("attachment; filename={0}.xls", "顶板离层数据查询"));
        ...
        ////模板文件  
        //string TempletFileName = Server.MapPath("~/template/dingbanlicengshujuchaxun.xlsx");//路径 
        ////导出文件  
        //string dts = DateTime.Now.ToString("yyyyMMddHHmmss");
        //string ReportFileName = Server.MapPath("~/xiazai/dingbanlicengshujuchaxun" + dts + ".xlsx");
        ...
        //    mySheet.Cells[2 + i, 6] = ds.Tables[0].Rows[i]["Displacement1"].ToString();
        //    mySheet.Cells[2 + i, 7] = ds.Tables[0].Rows[i]["Displacement2"].ToString();
        //    mySheet.Cells[2 + i, 8] = ds.Tables[0].Rows[i]["PointdepthA"].ToString();
        //    mySheet.Cells[2 + i, 9] = ds.Tables[0].Rows[i]["PointdepthB"].ToString();
        //    mySheet.Cells[2 + i, 10] = ds.Tables[0].Rows[i]["ABSWY"].ToString();
        //    mySheet.Cells[2 + i, 11] = ds.Tables[0].Rows[i]["AlarmH"].ToString();
        //}
