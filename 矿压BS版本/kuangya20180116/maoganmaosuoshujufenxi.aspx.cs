...
public partial class maoganmaosuoshujufenxi : System.Web.UI.Page
{
    ...
    protected void lbtn_export_Click(object sender, EventArgs e)
    {
        ...
        Response.AddHeader("Content-Disposition", "attachment; filename=锚杆锚索数据分析.jpg");
    ...
    protected void ddl_cequ_changed(object sender, EventArgs e)
    {
        ...
        if (ds.Tables[0].Rows.Count > 0)
        {
            ...
            sql = "SELECT Location FROM BoltSenInfo where areaName='" + ddl_cequ.SelectedValue + "' and roadwayName='" + ds.Tables[0].Rows[0]["roadName"].ToString() + "' and mgtype='"+ddl_maoguleixing.SelectedValue+"' order by Location";
    ...
    {
        string sql = "SELECT Location FROM BoltSenInfo where areaName='" + ddl_cequ.SelectedValue + "' and roadwayName='" + ddl_roadway.SelectedValue + "' and mgtype='" + ddl_maoguleixing.SelectedValue + "' order by Location";
        DataSet ds = DB.ExecuteSqlDataSet(sql, null);
        if (ds.Tables[0].Rows.Count > 0)
        {
            ddl_anzhuangweizhi.Items.Clear();
